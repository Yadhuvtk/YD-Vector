...
CHECKPOINT_DIR = "./YD_Vector_Checkpoints/checkpoint-43000"
PROJECTOR_RESUME = "yd_projector.bin"
OUTPUT_DIR = "./YD_Vector_Checkpoints"
SHARDS_PATH = (
    "file:E:/Yadhu Projects/YD-Vector/SVG_Shards/yd-vector-{000000..000226}.tar"
)
BATCH_SIZE = 10
LEARNING_RATE = 1e-4
...
class YDProjector(nn.Module):
    def __init__(self, vision_dim=1152, model_dim=4096):
        ...
        self.projector = nn.Sequential(
            nn.Linear(vision_dim, model_dim), nn.GELU(), nn.Linear(model_dim, model_dim)
        )
...
def transform_data(sample):
    ...
    img_tensor = (
        torch.tensor(list(img.getdata())).view(384, 384, 3).permute(2, 0, 1).float()
        / 255.0
    )
...
if __name__ == "__main__":
    ...
    print(f"Resuming from Step 43000...")
    ...
    dataset = (
        wds.WebDataset(SHARDS_PATH).shuffle(1000).decode("pil").map(transform_data)
    )
    loader = DataLoader(dataset, batch_size=BATCH_SIZE, num_workers=2)
    optimizer = torch.optim.AdamW(
        list(model.parameters()) + list(projector.parameters()), lr=LEARNING_RATE
    )

    print(f"\nTraining Restarted. Current Step: 43000")
    print(f"GPU Rest: {COOLDOWN_AFTER_CHECKPOINT} min break after each checkpoint\n")
    model.train()

    for step, (images, svg_texts) in enumerate(loader):
        actual_step = step + 43000
        ...
        with torch.amp.autocast("cuda", dtype=dtype):
            ...
            img_labels = torch.full((BATCH_SIZE, image_embeds.shape[1]), -100).to(
                "cuda"
            )
        ...
        if actual_step % 5 == 0:
            ...
            print(
                f"Step {actual_step} | Loss: {loss.item():.4f} | VRAM: {torch.cuda.memory_reserved()/1e9:.2f}GB{temp_str}"
            )
        ...
        if actual_step % 500 == 0:
            ...
            torch.save(
                projector.state_dict(), f"{OUTPUT_DIR}/projector-{actual_step}.bin"
            )
            ...
            with torch.no_grad():
                ...
                with open(
                    f"{OUTPUT_DIR}/preview-{actual_step}.svg", "w", encoding="utf-8"
                ) as f:
                    f.write(preview_svg)
